...
def get_astro_logic(dob, tob):
    ...
    try:
        ...
        sun = next((z[2] for i, z in enumerate(zodiacs) if (m == z[1] and d >= z[0]) or (m == (zodiacs[i-1][1]) and d < z[0])), "Soul")
...
def get_oracle_guidance(name, astro, stress, domain, problem):
    ...
    if stress >= 8: color = "#4b0000" # Deep Garnet

    elif 4 <= stress <= 7: color = "#1c1c3c" # Midnight Blue

    else: color = "#004d40" # Forest Green
    ...
    return f"""
        ...
        <div class="oracle-card" style="background-color: {color};">
            ...
            <h1 style='color: white; margin-bottom: 5px;'>THE ORACLE HAS SPOKEN # 

            <p style='opacity: 0.8;'>Target: {name} [{astro}] | Time: {now}

            <hr style='border-color: rgba(255,255,255,0.1);'>

            <h3 style='color: #ffd700;'>SITUATION: {problem}

            <p style='font-size: 1.2em;'>**THE GUIDANCE:** {guidance}

            <p style='font-size: 1.2em;'>**THE NEXT STEP:** Do the action. Do not analyze the result yet. Just move.
...
with col2:
    ...
    stress_val = st.select_slider("", options=list(range(11)), value=5)
